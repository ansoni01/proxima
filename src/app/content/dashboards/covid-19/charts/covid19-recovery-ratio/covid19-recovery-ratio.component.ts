import { Component, Input, SimpleChanges, OnChanges, NgZone, AfterViewInit } from '@angular/core';
import { GlobalMetric } from '../../models/metrics';
import { BaseComponent, Logger, StyleService } from '@core';
...
export class Covid19RecoveryRatioComponent extends BaseComponent implements OnChanges, AfterViewInit {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes.metrics && changes.metrics.currentValue) {
      this.metricsToDisplay = changes.metrics.currentValue as GlobalMetric[];
    }
  }
  ...
  loadChart(metrics: GlobalMetric[]) {
    this.zone.runOutsideAngular(() => {
      ...
      const confirmed = metrics.find((metric) => metric.type === 'confirmed');
      const data = metrics.filter((metric) => metric.type !== 'confirmed').reverse();
  ...
  private configureChart(chart: am4charts.RadarChart) {
    ...
    valueAxis.renderer.labels.template.fill = am4core.color(this._styles.palette.colorGray);
    ...
    chart.tooltip.label.fill = am4core.color(this._styles.palette.colorGray);
  }
