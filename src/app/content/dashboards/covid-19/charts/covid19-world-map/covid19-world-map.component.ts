import { Component, OnInit, Input, AfterViewInit, NgZone, OnChanges, SimpleChanges } from '@angular/core';
...
export class Covid19WorldMapComponent implements OnChanges, OnInit, AfterViewInit {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.selectedCountry && changes.selectedCountry.currentValue) {
      if (this.selectedCountry.code === 'global') {
        ...
        const selected = this.worldSeries.getPolygonById(this.selectedCountry.code);
  ...
  ngAfterViewInit() {
    if (this.countries) {
      this.cumulative = this.countries.reduce(
        (acc, current) => {
          const confirmed = acc.confirmed + Math.round(current.confirmed.today);
          const recovered = acc.recovered + Math.round(current.recovered.today);
          const deaths = acc.deaths + Math.round(current.deaths.today);
  ...
  rollOverCountry(mapPolygon: am4maps.MapPolygon) {
    ...
    if (mapPolygon) {
      ...
      if (image) {
        (<any>image.dataItem.dataContext).name = (<any>mapPolygon.dataItem.dataContext).name;
        image.isHover = true;
      }
    }
  }
  ...
  getZoomLevel(mapPolygon: am4maps.MapPolygon) {
    ...
    return Math.min(this.map.seriesWidth / (w * 2), this.map.seriesHeight / (h * 2));
  }
  ...
  configureMap(map: am4maps.MapChart, countries: CountryMetric[]) {
    ...
    const hasConfirmed = countries.filter((country) => country.confirmed.today > 0);
    ...
    worldPolygon.events.on('over', (e) => this.handleCountryOver.call(this, e));
    ...
    polygonHoverState.properties.fill = am4core.color(this._styles.GrayScale.gray300);

    const polygonActiveState = worldPolygon.states.create('active');
    polygonActiveState.properties.fill = am4core.color(this._styles.GrayScale.gray400);
  ...
  addBubbles(map: am4maps.MapChart, data: any[], worldSeries: am4maps.MapPolygonSeries) {
  ...
  addButtons(map: am4maps.MapChart) {
    ...
    const nameAndButtonsContainer = map.chartContainer.createChild(am4core.Container);
  ...
  changeDataType(name: string) {
    ...
    this.bubbleSeries.heatRules.getIndex(0).maxValue = this.max[currentType];
