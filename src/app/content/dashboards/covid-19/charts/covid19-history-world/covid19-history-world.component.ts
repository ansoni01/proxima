...
import { StyleService, BaseComponent } from '@core';
...
export class Covid19HistoryWorldComponent extends BaseComponent implements OnInit {
  ...
  constructor(private _covid: Covid19Service, private zone: NgZone, private _styles: StyleService) {
    super();
  }
  ...
  configure(chart: am4charts.XYChart) {
    ...
    dateAxis.max = this.currentDate.getTime() + am4core.time.getDuration('day', 5);
    ...
    chart.legend.labels.template.fill = am4core.color(this._styles.palette.colorDark);
    ...
    this.createSeries(chart, 'recovered', this._styles.palette.colorSuccess);
  ...
  updateSeriesTooltip(chart: am4charts.XYChart, dateAxis: am4charts.DateAxis) {
    ...
    chart.series.each((series) => {
      if (!series.isHidden) {
        ...
        series.showTooltipAtDataItem(series.tooltipDataItem as am4charts.XYSeriesDataItem);
      }
    });
  }
}
