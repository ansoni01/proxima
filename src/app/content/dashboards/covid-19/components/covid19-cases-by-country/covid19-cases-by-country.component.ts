...
import { faArrowCircleUp, faArrowCircleDown, faStopCircle, faSearch } from '@fortawesome/free-solid-svg-icons';
...
export class Covid19CasesByCountryComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._covid
      ...
      .subscribe((metrics) => {
        ...
        this.metrics.sort((a, b) => b.confirmed.today - a.confirmed.today);

        this.world = {
          name: 'World',
          code: 'global',
          confirmed: {
            today: this.metrics.reduce((sum, current) => sum + Math.round(current.confirmed.today), 0),
            yesterday: this.metrics.reduce((sum, current) => sum + Math.round(current.confirmed.yesterday), 0),
          },
  ...
  getVariationIcon(value: MetricValue) {
    ...
    return variation == 0 ? this.icons.unvariant : variation > 0 ? this.icons.up : this.icons.down;
  }
