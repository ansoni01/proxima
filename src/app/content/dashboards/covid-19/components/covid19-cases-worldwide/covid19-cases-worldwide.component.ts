...
export class Covid19CasesWorldwideComponent extends BaseComponent implements OnChanges, OnInit {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.metrics && changes.metrics.currentValue) {
      ...
      this.metricsToDisplay = metrics.filter((metric) => metric.type !== 'confirmed');
