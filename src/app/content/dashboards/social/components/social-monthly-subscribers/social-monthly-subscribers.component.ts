...
import { BaseComponent, StyleService } from '@core';
...
export class SocialMonthlySubscribersComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  initializeCharts() {
    const ctx = (this.mainChart.nativeElement as HTMLCanvasElement).getContext('2d');
    const data = [300, 450, 200, 280, 250, 400, 350, 300, 480, 250, 450, 500];
        ...
        datasets: [
          {
            ...
            backgroundColor: this._styles.toRgba(this._styles.brands.facebook, 0.14),
            borderColor: this._styles.brands.facebook,
          },
          {
            label: 'Instagram',
            data: data.map((value) => value * 3),
            backgroundColor: this._styles.toRgba(this._styles.brands.instagram, 0.14),
            borderColor: this._styles.brands.instagram,
          },
          {
            label: 'Youtube',
            data: data.map((value) => value * 1),
            backgroundColor: this._styles.toRgba(this._styles.brands.youtube, 0.14),
        ...
        tooltips: {
          callbacks: {
            labelColor: function (tooltipItem, chart) {
              const color = chart.data.datasets[tooltipItem.datasetIndex].borderColor as string;
