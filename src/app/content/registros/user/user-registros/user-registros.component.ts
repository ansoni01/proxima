...
import { FileUploadStatus } from '@core/models/file-upload.status';


@Component({
  selector: 'prx-user-registros',
  templateUrl: './user-registros.component.html',
  styleUrls: ['./user-registros.component.scss'],
})
export class UserRegistrosComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.user = this.authenticationServices.getUserFromLocalCache();
    console.log(this.authenticationServices.getUserFromLocalCache())
    this.getUsers(true);
  ...
  public onUpdateCurrentUser(user: User): void {
    this.refreshing = true;
    this.currentUsername = this.authenticationServices.getUserFromLocalCache().username;
    const formData = this.userService.createUserFormDate(this.currentUsername, user, this.profileImage);
    this.subscriptions.push(
      this.userService.updateUser(formData).subscribe(
        (response: User) => {
          this.authenticationServices.addUserToLocalCache(response);
          this.getUsers(false);
          this.fileName = null;
          this.profileImage = null;
          // this.sendNotification(NotificationType.SUCCESS, `${response.firstName} ${response.lastName} updated successfully`);
  ...
  private getUserRole(): string {
    return this.authenticationServices.getUserFromLocalCache().role;
  }
