...
export class QuillReactiveFormsComponent extends BaseFormComponent implements OnInit {
  ...
  ngOnInit() {
    this.form.controls.editor.valueChanges.pipe(debounceTime(400), distinctUntilChanged()).subscribe((data: any) => {
      // native fromControl value changes with debounce
    });

    this.quillEditor.onContentChanged.pipe(debounceTime(400), distinctUntilChanged()).subscribe((data: any) => {
  ...
  setControl() {
    this.form.setControl('editor', new FormControl('This new value is set to the editor.'));
  }
