...
import { StyleService, ColorScheme } from '@core';
...
export class UiColorsComponent implements OnInit {
  ...
  constructor(private _styles: StyleService) {
    ...
    const flatColors = allColors.filter((color) => color.name === 'GrayScale')[0];
    const palette = allColors.filter((color) => color.name === 'palette')[0];
    const colors = allColors.filter((color) => color.name.startsWith('Color'));

    palette.value = (palette.value as ColorValue[]).map((color: ColorValue) => {
      return { name: color.name.substring(5, color.name.length), value: color.value };
    });
