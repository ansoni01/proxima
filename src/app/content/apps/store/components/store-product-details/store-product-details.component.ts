...
export class StoreProductDetailsComponent implements OnInit {
  ...
  networks = [{ icon: faFacebook }, { icon: faTwitter }, { icon: faInstagram }, { icon: faPinterest }];
  ...
  constructor(private _products: StoreProductService, private _activatedRoute: ActivatedRoute) {}

  ngOnInit(): void {
    this._activatedRoute.paramMap.subscribe((params) => {
      this.productSlug = params.get('id');

      this._products.getProductDetails(this.productSlug).subscribe((product) => {
