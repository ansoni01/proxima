...
export class NgxDtFilteringComponent extends BaseDataTable implements OnInit {
  ...
  constructor(private _users: UsersService, private formBuilder: FormBuilder) {
    super();
  ...
  updateFilter({ value }: { value: any }) {
    ...
    const filtered = this.users.filter((user: User) => {
      return (
        user.name.toLowerCase().indexOf(value.name.toLowerCase()) !== -1 &&
        user.location.country.toLowerCase().indexOf(value.country.toLowerCase()) !== -1
      );
    });
